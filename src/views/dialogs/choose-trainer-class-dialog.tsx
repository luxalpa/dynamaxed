...
class ChooseTrainerClassDialogCmp extends Vue {
  ...
  render() {
    return (
      <v-card><v-container><div class="trainer-class-grid">{
        Object.entries(GameModel.model.trainerClasses).map(([key, props]) => (
          <v-btn class="ma-1"onclick={() => DialogManager.accept(key)}>{props.name}</v-btn>
        ))
